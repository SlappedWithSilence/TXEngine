...
public class CombatEntity implements CombatAgency, Components.Tabable {
    ...
    public Pair<Ability, Item> makeChoice(CombatEngine engine) {
        ...
        if (this.getResourceManager().resourcePercentage(Manager.primaryResource) <= PRIMARY_RESOURCE_TOLERANCE) {
            ...
            if (healingItems.size() > 0) return new Pair<>(null, Manager.itemManager.get_instance(healingItems.get(new Random().nextInt(healingItems.size())))); // TODO: Verify that this won't go out of bounds
